from openskistats.sunlight import get_typical_ski_season_dates


def test_get_typical_ski_season_dates__solstice() -> None:
    north_solstice_open, north_solstice_close = get_typical_ski_season_dates(
        "north", "solstice"
    )
    assert north_solstice_open == north_solstice_close
    south_solstice_open, south_solstice_close = get_typical_ski_season_dates(
        "south", "solstice"
    )
...
def test_get_typical_ski_season_dates__season() -> None:
    north_season_open, north_season_close = get_typical_ski_season_dates(
        "north", "season"
    )
    north_season_duration = (north_season_close - north_season_open).days
    south_season_open, south_season_close = get_typical_ski_season_dates(
        "south", "season"
    )
